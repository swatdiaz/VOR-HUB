...
| Catch 1 Billion Ducks adapter | `games/catch_1_billion_ducks.lua` | One-button lobby chores, verified solo queueing, cross-place resume, server-credited duck and boss targeting, selling, weapon/cash progression, dogs, crates, achievements, and telemetry. |
...
| Sniper Arena adapter | `games/sniper_arena.lua` | Native shot-path silent aim, cursor aimbot, zero-delay event-driven trigger assist, late-frame player-and-bot collider expansion with native hitbox targeting, recoil/spread/reload controls, team-filtered optional enemy ESP, server-checked kill unlocks, owned loadouts, ready reward claims, matchmaking, and live progression telemetry. |
